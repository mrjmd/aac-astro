...
1. **Engineering assessment** — a structural engineer determines the current load, required column capacity, and footing requirements
2. **Temporary shoring** — temporary supports are installed adjacent to the failing column to carry the load during replacement
3. **Old column removal** — the failed column is cut out
4. **Footing inspection or installation** — if the existing footing is inadequate, a new reinforced concrete footing is poured and cured
5. **New column installation** — a properly sized, concrete-filled steel lally column is set on the footing and shimmed tight to the beam
6. **Load transfer** — temporary supports are carefully removed, transferring the load to the new column
7. **Final inspection** — beam deflection is checked and the installation is verified
